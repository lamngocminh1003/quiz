...
import SearchAllFileByName from "../SearchOther/SearchAllFileByName";
import SearchAllFileByDate from "../SearchOther/SearchAllFileByDate";
import SearchAllFileByDateWindow from "../SearchOther/SearchAllFileByDateWindow";
import { searchAllFilesByWindow } from "../../services/fileService";
...
const AllFiles = (props) => {
  ...
  const fetchAllFiles = async () => {
    try {
      setIsLoading(true);
      let res = await searchAllFilesByWindow(startTime, endTime, sortOptions);
                    ...
                    {listFiles && listFiles.length > 0 ? (
                      <>
                        {listFiles.map((item, index) => {
                          let activationData = new Date(item.activationTime);
                          ...
                          const displayText =
                            permission === 0 ? "Quản trị viên" : "Khoa/ phòng";
                          const displayIcon =
                            permission === 0 ? (
